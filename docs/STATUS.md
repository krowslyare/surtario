...
| Ingredient-list research | Implemented on separate PR branch; not deployed | Photo/PDF AI reading, reviewed provenance, one/many selection, durable two-lane research and independent Overview results. PR #43 review corrections distinguish replacement from retry, validate replay metadata and show each batch case once. Local real PNG/PDF + four-ingredient Firecrawl/OpenAI acceptance; details in [VERIFICATION](VERIFICATION.md) |
