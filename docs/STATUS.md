...
| Ingredient-list continuity | Implemented on follow-up branch; not deployed | Pending rows remain available after a partial launch; uncertain AI readings reuse their request ID; available findings lead the row while the round-limit notice remains explicit. 19 focused browser tests passed, followed by 5/5 after correcting partial-launch navigation from Explore; the hosting build passed. |
