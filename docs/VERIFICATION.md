...
### PR #44 review correction before integration
...
### PR #45 publication and completed hosted recovery

PR #45 merged as `ffbdde8`. [Verify run 35680145456](https://github.com/krowslyare/restaurant-procurement/actions/runs/35680145456) passed **343 tests in 53 files**, published the development backend/frontend and verified all 37 hosted files. Retrying the exact failed UI draft succeeded without editing its ingredient or source. Reload restored the saved comparison, its 40 lb scenario, organic specification, USD 42.99 / 25 lb source and unresolved terms. Overview changed that ingredient's action to “Review comparison”; opening it recovered the same comparison identifier.
