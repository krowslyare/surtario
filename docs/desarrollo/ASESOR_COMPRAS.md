...
Local verification: 110 domain/backend tests, typecheck/build and eight focused browser cases passed against an isolated local Convex backend. Tests cover ownership, immutable retries, stale revisions, provider-disabled behavior, numeric/source validation, budget/currency boundaries, recovery after reload and mobile/desktop layout. Independent Sol review identified removed-offer evidence and mixed-currency budget issues; both were corrected with regressions. That original delivery did not execute a provider. Later local Luna CLI tool evidence and the remaining direct OpenAI API boundary are recorded below.


## Guided save and analysis — September 10, 2026

The executive verdict now precedes detailed offers and mail tools. One explicit action saves the current comparison and decision context before preparing a scenario and, when enabled, requesting AI interpretation. Research still needs no quantity; advisor analysis requires a positive quantity. Pending or invalid inputs cannot trigger a paid model call.

Opening a comparison recovers its newest semantically matching scenario. The server reuses an existing scenario for the same revision/context, including calculated, running, completed and failed states; a reload or a new client retry ID does not create another model call. Changing request, offers, evidence or selection invalidates the old result. Async completion is guarded against edits, replacement drafts and unmounts.

In computer use, the real Luna CLI explanation followed the actual `evaluateScenarios` and `readEvidence` tools: a single eligible synthetic offer required PEN 94 for 20 kg, and the verdict asked for another verifiable offer instead of claiming savings. Reloading recovered that analysis without another model request. Direct OpenAI API and hosted acceptance remain pending. Integrated validation: [DEMO_FLOW_VALIDATION.md](DEMO_FLOW_VALIDATION.md).
