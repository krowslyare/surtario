...
Independent Luna max review identified one P2: the recap offered saving even when blocked site storage prevented session initialization. Saving now follows the existing session/query/connection readiness, with an explanation when unavailable. A browser regression verifies that a confirmed answer still recalculates locally while the save button remains disabled. The parent reviewed and tested this correction.
