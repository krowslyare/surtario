...
export default function App() {
  ...
  const [quantity, setQuantity] = useState(String(riceRequest.quantity));
  const [offers, setOffers] = useState<SupplierOffer[]>(() =>
    riceOffers.map((offer) => ({ ...offer })),
  );
  ...
  function reset() {
    setRequest({ ...riceRequest });
    setQuantity(String(riceRequest.quantity));
    setOffers(riceOffers.map((offer) => ({ ...offer })));
    setSources(initialSources());
    setModal(null);
    setMessage("Ejemplo restaurado.");
  }
  ...
  return (
    <>
      ...
      <header className="topbar">
        ...
        <button className="button text-button" onClick={() => setModal("help")}>
          <CircleHelp size={18} />
      ...
      <main>
        <div className="workspace-nav">
          ...
          <span className="demo-badge">Ejemplo sintético</span>
        </div>
        <div className="page-title">
          <div>
            <h1>Compara antes de comprar</h1>
            <p>El precio por kilo es solo una parte de la cuenta.</p>
          </div>
          <button
      ...
      {modal === "reset" && (
        <Dialog title="¿Restaurar el ejemplo?" onClose={() => setModal(null)}>
          <p>
            Se reemplazarán las ofertas y la cantidad de esta vista por el
            ejemplo inicial de arroz.
          </p>
          <div className="dialog-actions">
