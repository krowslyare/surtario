...
test("respuesta vinculada se revisa como oferta nueva y se recupera con su correo original", async ({
  page,
  context,
}) => {
  ...
  await page.getByRole("button", { name: "Revisar como nueva oferta" }).click();
  ...
  await dialog.getByLabel("Especificación", { exact: true }).fill("Blanco");
  await dialog
  ...
  await dialog
    .getByRole("button", { name: "Continuar con nueva oferta" })
    .click();
  await expect(
